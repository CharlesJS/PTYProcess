...
extension PTYProcess {
    public convenience init(executableURL: URL, arguments: [String] = [], environment: [String : String]? = nil) throws {
        guard executableURL.isFileURL else {
            throw POSIXError(.EINVAL)
        }

        if #available(macOS 11.0, macCatalyst 14.0, iOS 14.0, tvOS 14.0, watchOS 7.0, *) {
            self.init(executablePath: FilePath(executableURL.path), arguments: arguments, environment: environment)
        } else {
            self.init(executablePath: executableURL.path, arguments: arguments, environment: environment)
        }
    }
